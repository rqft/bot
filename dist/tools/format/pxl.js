...
async function ajit(context, args) {
    const { target } = args;
    const { payload: image } = await exports.instance.ajit([target]);
...
async function emojiMosaic(context, args) {
    ...
    const { payload: image } = await exports.instance.emojiMosaic([target], args["group-size"], scale);
...
async function eyes(context, args) {
    ...
    const { payload: image } = await exports.instance.eyes([target], type);
...
async function flag(context, args) {
    ...
    const { payload: image } = await exports.instance.flag([target], flag, opacity);
...
async function glitch(context, args) {
    ...
    const { payload: image } = await exports.instance.glitch([target], iterations, amount, {
...
async function imageSearch(context, args) {
    const { query } = args;
    const { payload: data } = await exports.instance.imageSearch(query, pariah_1.APIs.PxlAPI.SafeSearch.STRICT, true);
...
async function jpeg(context, args) {
    ...
    const { payload: image } = await exports.instance.jpeg([target], quality);
...
async function lego(context, args) {
    const { target, scale } = args;
    const { payload: image } = await exports.instance.lego([target], args["group-size"], scale);
...
async function screenshot(context, args) {
    ...
    const { payload: image } = await exports.instance.screenshot(url.toString(), {
        browser,
        locale,
        theme,
...
async function snapchat(context, args) {
    ...
    const { payload: image } = await exports.instance.snapchat([target], filter);
...
async function sonic(context, args) {
    const { text } = args;
    const { payload: image } = await exports.instance.sonic(text);
...
async function thonkify(context, args) {
    const { text } = args;
    const { payload: image } = await exports.instance.thonkify(text);
...
async function webSearch(context, args) {
    const { query } = args;
    const { payload: data } = await exports.instance.webSearch(query, pariah_1.APIs.PxlAPI.SafeSearch.STRICT);
    const paginator = new paginator_1.Paginator(context, {
