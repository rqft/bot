...
function logBlacklistedUserAction(message) {
    config_1.config.logs.blacklist.userBlocked.forEach((e) => {
        ...
        ch.send(`:warning: Blacklisted User **${message.author}** ${formatID_1.formatID(message.author.id)} tried to use command \`${message.cleanContent}\` in ${channelName} ${formatID_1.formatID(message.channel.id)} ${guildName} ${message.guild ? formatID_1.formatID(message.guild.id) : ""}`);
    });
}
