...
function logCommandError(message, error) {
    config_1.config.logs.commands.onError.keys.forEach((e) => {
        ...
        ch.send(`:x: **${message.author}** ${formatID_1.formatID(message.author.id)} tried to use command \`${message.cleanContent}\` in ${channelName} ${formatID_1.formatID(message.channel.id)} ${guildName} ${message.guild ? formatID_1.formatID(message.guild.id) : ""} and caused an error.
\`\`\`ts
${error}
\`\`\``);
    });
