...
export async function ajit(
  ...
  const image = await instance.ajit([target]);
...
export async function emojiMosaic(
  ...
  const image = await instance.emojiMosaic([target], args["group-size"], scale);
...
export async function eyes(
  ...
  const image = await instance.eyes([target], type);
  const embed = await Embed.image(
    context,
    image,
    `eyes-${type}.png`,
...
export async function flag(
  ...
  const image = await instance.flag([target], flag, opacity);
  const embed = await Embed.image(
    context,
    image,
    `flag-${flag}.png`,
...
export async function glitch(
  ...
  const image = await instance.glitch([target], iterations, amount, {
...
export async function imageSearch(
  ...
  const { query } = args;
  const data = await instance.imageSearch(
    query,
...
export async function jpeg(
  ...
  const image = await instance.jpeg([target], quality);
...
export async function lego(
  ...
  const image = await instance.lego([target], args["group-size"], scale);
...
export async function screenshot(
  ...
  const image = await instance.screenshot(url.toString(), {
    browser,
    locale,
    theme,
...
export async function snapchat(
  ...
  const image = await instance.snapchat([target], filter);
...
export async function sonic(
  ...
  const image = await instance.sonic(text);
...
export async function thonkify(
  ...
  const image = await instance.thonkify(text);
...
export async function webSearch(
  ...
  const data = await instance.webSearch(query, APIs.PxlAPI.SafeSearch.STRICT);
  const paginator = new Paginator(context, {
