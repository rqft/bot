...
export async function storeImage(
  ...
  {
    const imagescript = await decode(value);
    const encoded = await imagescript.encode(undefined, {
      source: "rqft.space",
      comment: "Generated by Vybose (https://rqft.space)",
...
export function escapeRegExp(payload: string) {
  return payload.replace(/[|\\{}()[\]^$+*?.]/g, "\\$&").replace(/-/g, "\\x2d");
}
export function unstringify(payload: string): Value {
  try {
...
export function formatBytes(bytes: number, decimals = 2, si: boolean = true) {
  if (bytes === 0) return "0 Bytes";

  const delimiter = si ? 1000 : 1024;
  const dm = decimals < 0 ? 0 : decimals;
  const sizes = si ? ByteUnits : SIByteUnits;
...
export function getDuplicates<T>(payload: Array<T>): Array<T> {
  const duplicates: Array<T> = [];
  const set = new Set<T>();
  for (const item of payload) {
    if (set.has(item)) duplicates.push(item);
    set.add(item);
  }
  return duplicates;
}
