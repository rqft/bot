...
import { decode, Frame, Image } from "imagescript/";
export async function imageFlop(
  ...
  if (url) {
    ...
    let editor = await decode(data);

    if (editor instanceof Image) {
      editor = mirror(editor, method);
    } else {
      for (let index = 0; index < editor.length; index++) {
        const frame = editor[index]!;
        editor[index] = mirror(frame, method);
      }
    }
